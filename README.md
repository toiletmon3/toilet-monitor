...
apps/
  web/      → React 18 + Vite + Tailwind CSS (PWA)
              ...
              ├── /cleaner             — אפליקציית עובד (משימות + משוב חיובי)
              ├── /supervisor          — ממשק אחראי משמרת (זהה לעובד + תגית)
              └── /admin               — לוח בקרה מנהל (Dashboard, Analytics, Escalation, Settings)
...
### 🛡️ Shift Supervisor (אחראי משמרת)
...
- **Overview Dashboard בסגנון Case-Study** (BuzzzTech-inspired) — תצוגה אחת לכל המידע בזמן אמת:
  - **שורת KPI כפולה** — תקופה קודמת (sparklines רגועים) מול הטווח הנבחר (כרטיסים צבעוניים בולטים + גרף-קו עם תוויות ערכים + חץ מגמה ירוק/אדום)
  - **2 דונאטים** — כללי (שביעות רצון / ניקיון / תחזוקה) + פירוק תקלות ניקיון לפי סוג
  - **Performance at a Glance** — שורת 6 KPIs (ציון · תקלות · אינטראקציות · זמן תגובה · % תקלות · זמן שנחסך) + גרף משולב (בארים יומיים צבועים לפי tier + קו אדום של תקלות עם תוויות שיא) + דונאט קטן של הבעיה המובילה
  - **טבלת תאים + ציון** — מיון מהגרוע ביותר, מתג טבלה/כרטיסים, מונה תאים
  - **Deep Dive** — טבלה רחבה לכל תא: אינטראקציות · תקלות · בעיה מובילה (עם פס %) · הגעות עובדים · אחראי משמרת · זמן תגובה · שביעות רצון (עם פס) · ציון בריאות
  - **סינון מדורג** — בניין → קומה → תא · בורר טווח 7/30/90 יום · עדכון בזמן אמת (WebSocket)
- Analytics מלא: SLA · תדירות · דפוסים · עומסים שעתיים · לפי יום · ביצועי עובדים
- **דו"ח יומי במייל למנהל** — נשלח אוטומטית ב-08:00 (Asia/Jerusalem) וכולל:
  - **סקירה כללית 30 יום** — 4 כרטיסי KPI עם chip מגמה (ציון ממוצע / תקלות / זמן תגובה / זמן שנחסך) מול 30 הימים הקודמים
  - פס פילוח כללי (שביעות רצון / ניקיון / תחזוקה) + טבלת ציון לכל תא (הגרועים ראשונים)
  - **פעילות אתמול** (מתחת לסקירה) — סטטיסטיקות יומיות, תקלות נפוצות, נקודות חמות, ביצועי עובדים, עובדים שנכחו ולא טיפלו
- **ציון בריאות לכל תא שירותים (0-100, גבוה = טוב)** — מתחיל מ-100 ויורד לפי 4 ניכויים משוקללים:
  - **תדירות תקלות (40%)** — מספר קריאות בתא, מנורמל מול התא העמוס ביותר
  - **חומרת התקלות (25%)** — ממוצע `priority` של סוגי תקלה (1=קריטי, 5=שולי)
  - **זמן תגובה (20%)** — ממוצע דקות מפתיחה לסגירה, חתוך ב-60 דק'
  - **חזרתיות (15%)** — % תקלות שחזרו על עצמן באותו סוג תוך 24ש'
  - 3 רמות צבע: 🟢 תקין (≥70) · 🟠 בעייתי (40-69) · 🔴 קריטי (<40)
  - מוצג גם בדשבורד הראשי וגם בעמוד Analytics + מיוצא ל-PDF/Excel · API: `GET /analytics/restroom-scores`
