...
## 3️⃣ DuckDNS (הדומיין הישן — הוצא משימוש)

> ⛔ **הדומיין הזה הוצא משימוש (07/2026).** הדומיין היחיד הוא `cleanco.ai`.
> ה-nginx חוסם (444) כל בקשה ל-duckdns. מומלץ למחוק את הרשומה בחשבון
> DuckDNS ואת סקריפט עדכון ה-IP בשרת (`/root/duckdns/duck.sh` + crontab).
