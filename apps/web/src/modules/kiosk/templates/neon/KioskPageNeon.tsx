/**
 * KioskPageNeon — alternative kiosk UI based on Figma Make design.
 * Black background with cyan neon borders + Lucide icons with glow.
 *
 * Not wired to routes by default. To use:
 *   1. In apps/web/src/routes.tsx (or wherever <KioskPage/> is rendered)
 *      replace the import with:  import KioskPage from '.../templates/neon/KioskPageNeon'
 *   2. Also swap KioskButton imports in this file to:
 *      import KioskButton from './KioskButtonNeon'
 *
 * All other kiosk screens (confirmation, cleaner check-in, selector)
 * remain the default ones.
 */
...
import { Scroll, Sparkles, Trash2, Droplets, Wrench, Hand, Smile, Clock, Timer, Wind, ShowerHead, SmilePlus, Star, Bell, AlertCircle } from 'lucide-react';
...
export default function KioskPageNeon() {
  ...
  return (
    <div className="kiosk-root flex flex-col overflow-hidden" style={{ height: '100dvh', minHeight: '-webkit-fill-available', background: '#000000' }} dir="rtl">
      <div className="flex items-center justify-between px-6 pt-4 pb-1 flex-shrink-0">
        <div className="flex rounded-xl overflow-hidden" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(0,229,255,0.25)', padding: 2, gap: 2 }}>
          {([
      ...
      <div className="px-6 pt-3 pb-3 flex-shrink-0">
        <h1 className="text-white mb-3" style={{ fontSize: 'clamp(2rem, 4vw, 3.25rem)', fontWeight: 600, textShadow: '0 0 24px rgba(0,229,255,0.35)' }}>
          {t('kiosk.title')}
        </h1>
        {stats && (
        ...
        {posBtn && (() => {
          ...
          return (
            <div style={{ flex: '0 0 14%', minHeight: 90 }}>
              <KioskButton icon={meta.icon} color={meta.color} label={label} onPress={() => handleIssuePress('positive_feedback')} fullWidth />
            </div>
          );
        })()}

        <div className="flex-1 grid grid-cols-2 gap-4" style={{ gridTemplateRows: `repeat(${Math.ceil(gridBtns.length / 2)}, minmax(0, 1fr))`, minHeight: 0 }}>
          {gridBtns.map((btn) => {
            ...
            return (
              <KioskButton key={btn.code} icon={meta?.icon ?? FallbackIcon} color={meta?.color ?? NEON_CYAN} label={label} onPress={() => handleIssuePress(btn.code)} />
            );
          })}
        </div>
      </div>
